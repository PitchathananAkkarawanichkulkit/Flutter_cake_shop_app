...
import 'package:cake_shop_app/views/cake_shop_detail_ui.dart';
...
class _CakeShopListUiState extends State<CakeShopListUi> {
  //ตัวแปลสำหรับเก็บข้อมูลรูปภาพเค้ก
...
//ตัวแปนเก็บข้อมูลร้านเค้ก 7 ร้าน listview
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('มากินเค้กกันเถอะ',
            style: TextStyle(
              color: Colors.white,
              fontSize: 20,
          ...
          children: [
            ...
            Expanded(
              child: ListView.separated(
                itemCount: cakeShops.length, //จํานวนร้านใน listview
                //เส้นแบ่งร้าน
                ...
                itemBuilder: (context, index) => ListTile(
                  leading: ClipRRect(
                    borderRadius: BorderRadius.circular(10),
                    child: Image.asset(cakeShops[index].image1!,
                        width: 80, fit: BoxFit.cover),
                  ),
                  title: Text(cakeShops[index].name!),
                  subtitle: Text('เบอร์โทร ${cakeShops[index].phone!}'),
                  trailing: Icon(Icons.info, color: Colors.blue),
                  onTap: () {
                    //เปลี่ยนหน้าจอแบบย้อนกับได้
                    Navigator.push(
                      context,
                      MaterialPageRoute(
